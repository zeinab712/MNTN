function Section(Props) {
    return (
        ...
        class={`relative flex justify-center px-4 mb-[200px] sm:px-10 md:px-20 lg:px-40 flex flex-col md:flex-row items-center gap-6 sm:gap-10 md:gap-16 lg:gap-20 ${
            Props.dir === "right" ? "md:flex-row-reverse" : "md:flex-row"
        }`}
        id={Props.id}
        >
        <h1
            className={`absolute top-[-60px] sm:${Props.dir}-[50px] md:${Props.dir}-[80px] lg:${Props.dir}-[${Props.value}] font-bold z-[50] text-[120px] md:text-[130px] lg:text-[150px] text-white opacity-[0.1] pointer-events-none select-none ${Props.dir}-[20px]`}
        >
            {Props.secNum}
        </h1>
